...
if (isset($_POST['cambiar_email'])) {
    ...
    $nuevo_email = filter_var($nuevo_email, FILTER_SANITIZE_EMAIL);

    // Validar email completo (ej: usuario@gmail.com)
    if (filter_var($nuevo_email, FILTER_VALIDATE_EMAIL) !== false) {
        ...
        if ($stmt->execute()) {
            ...
            $mensaje = "Error SQL: " . $stmt->error . " — Código: " . $stmt->errno;
        }

            } else {
                $mensaje = "Ingresá un email válido (ejemplo: nombre@correo.com).";
            }
        }
...
<html lang="es">
    ...
    <body>
        ...
        <form method="POST">
            ...
            <input type="password" name="confirm_pass" placeholder="Confirmar nueva contraseña" required>
            <button type="submit" name="cambiar_contraseña">Actualizar Contraseña</button>
        </form>

        <!-- Form eliminar cuenta -->
        <form method="POST" id="formEliminarCuenta">
            <input type="hidden" name="eliminar_cuenta" value="1">
            <h2>Eliminar Cuenta</h2>
            <button type="submit" class="btn-eliminar-cuenta">Eliminar Cuenta</button>
        </form>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            formEliminar.addEventListener("submit", function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Esta acción eliminará tu usuario y todos tus datos permanentemente.",
                    icon: "warning",
